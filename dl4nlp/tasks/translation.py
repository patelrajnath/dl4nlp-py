...
class TranslationTask(FairseqTask):
    ...
    def load_dataset_online(self, lines_src, lines_tgt, split='train'):
        """Load a dataset split."""

        src_datasets = []
        tgt_datasets = []

        src_datasets.append(indexed_dataset.make_dataset(lines_src, impl="online",
                                                         fix_lua_indexing=True, dictionary=self.src_dict))
        tgt_datasets.append(indexed_dataset.make_dataset(lines_tgt, impl="online",
        ...
        if len(src_datasets) == 1:
            ...
            sample_ratios = [1] * len(src_datasets)
            sample_ratios[0] = self.upsample_primary
            src_dataset = ConcatDataset(src_datasets, sample_ratios)
            tgt_dataset = ConcatDataset(tgt_datasets, sample_ratios)
