...
def get_server_parser(interactive=False, default_task='translation'):
    parser = get_parser('nmt_server', default_task)
    add_dataset_args(parser, gen=True)
    add_generation_args(parser)
...
def add_server_args(parser):
    group = parser.add_argument_group('fairseq-server')
    group.add_argument('--host', default='127.0.0.1',
                       help='IP address to which nmt server to be started.')
    group.add_argument('--port', default='5000',
                       help='TCP/IP port at which nmt server to listen for the translation requests.')
    group.add_argument('--threads', default='1',
                       help='Number of fairseq decoders to be started.')
    group.add_argument('--adaptation', action='store_true',
                       help='Run on-the-fly adaptation if translation memory is available')
    group.add_argument('--gpu-ids', nargs='*', help='Set GPUs to be used e.g.: 0 1 2 3')
...
def add_preprocess_args(parser):
    ...
    parser.add_argument(
        '--max-vocabulary-size', dest='max_vocab_size', type=int,
        help='This is the maximum size of the vocabulary '
             '(default value is unbound).', default=None)
    parser.add_argument(
        '--vocabulary-prune', dest='vocab_pruning_threshold', type=float,
        help='This is the threshold to prune out subwords of the vocabulary '
